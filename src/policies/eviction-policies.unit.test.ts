...
describe('Eviction Policies', () => {
  ...
  describe('LRUPolicy', () => {
    ...
    it('should evict based on creation time when lastAccessed is the same', () => {
      ...
      const entries: [string, CacheState][] = [
        ['key1', {
          value: 'value1', createdAt: now - 300, updatedAt: now, accessCount: 1, lastAccessed: now,
        }],
        ['key2', {
          value: 'value2', createdAt: now - 200, updatedAt: now, accessCount: 1, lastAccessed: now,
        }],
        ['key3', {
          value: 'value3', createdAt: now - 100, updatedAt: now, accessCount: 1, lastAccessed: now,
        }],
      ];
  ...
  describe('LFUPolicy', () => {
    ...
    it('should evict based on lastAccessed time when access counts are the same', () => {
      ...
      const entries: [string, CacheState][] = [
        ['key1', {
          value: 'value1', createdAt: now, updatedAt: now, accessCount: 1, lastAccessed: now - 300,
        }],
        ['key2', {
          value: 'value2', createdAt: now, updatedAt: now, accessCount: 1, lastAccessed: now - 200,
        }],
        ['key3', {
          value: 'value3', createdAt: now, updatedAt: now, accessCount: 1, lastAccessed: now - 100,
        }],
      ];
